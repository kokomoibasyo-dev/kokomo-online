function evaluateYear(){
  ...
  const continuedBefore=continuedIds().slice();
  let delta={};
  ...
  continuedBefore.forEach(id=>{
    const p=policyById(id), fit=localFit(p);
  ...
  const nextActive=continuedBefore.slice(); S.selected.forEach(id=>{ const p=policyById(id); if(p.upkeep && !nextActive.includes(id)) nextActive.push(id); });
  const names=continuedBefore.map(id=>'継続:'+policyById(id).name).concat(S.selected.map(id=>'新規:'+policyById(id).name));
  S.active=nextActive.slice(0,3); S.keep={}; S.active.forEach(id=>S.keep[id]=true);
...
function runYear(){
  ...
  const result=evaluateYear(); renderTownSnapshot(); renderResultSection(result); renderHistory(); updateRunDock();
}
function nextYear(){
  if(S.year>=currentDuration()) return finishGame();
  S.year++; S.selected=[]; S.event=null; S.concerns=null; S.dilemma=null; S.currentBudgetBonus=S.nextBudgetBonus; S.nextBudgetBonus=0; S.resolved=false; S.keep={}; S.active.forEach(id=>S.keep[id]=true);
...
function getAdvisorHtml(){
  ...
  const topConcern=S.concerns?.[0]; const saturated=Object.entries(S.focusStreak).filter(([,v])=>v>=2).sort((a,b)=>b[1]-a[1])[0];
  const routes=[
    {title:'住民の声を取りにいく', body:'今年の関心1位「'+topConcern.name+'」に対応する施策を選ぶ。納得感は取りやすいが、長期目標の主戦場を外す可能性がある。'},
    {title:'長期目標を取りにいく', body:'「'+g.name+'」の中心指標（'+primaryKeys().map(k=>METRICS[k].n).join('・')+'）に効く施策を選ぶ。今年の声とのズレには注意。'},
    {title:'崩れやすい分野を守る', body:'いちばん遅れている「'+METRICS[lagging[0].k].n+'」や、地区バランス・納得感・疲労を見て、町全体の破綻を防ぐ。'}
  ];
  let html='';
  html += '<div class="advisorBlock"><h3>🔎 今年の読みどころ</h3><p>この年の最優先課題候補は <b>'+lagging.map(x=>METRICS[x.k].n).join('、')+'</b>。住民関心1位は <b>'+topConcern.icon+' '+topConcern.name+'</b> です。</p></div>';
  html += '<div class="advisorBlock"><h3>🗣️ 住民の声は何を意味する？</h3><p>「'+topConcern.name+'」への関心が高いのは、単に人気だからではなく、いまの'+c.name+'で <b>'+METRICS[topConcern.metric].n+'</b> が足りていないからかもしれません。ただし、今年の声に応えることと、長期目標へ最短で近づくことは同じではありません。</p></div>';
...
function renderAll(){
  ...
  renderTownSnapshot(); renderContinuations(); renderConcerns(); renderDilemma(); renderEventVoices(); renderSelectedSummary(); renderMetrics(); renderPolicies(); renderHistory(); updateRunDock();
}
