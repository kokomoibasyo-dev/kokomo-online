...
function slotsLeft(){return Math.max(0,MAX_POLICIES-totalPlanned())}
function trimSelections(){
  while(S.selected.length&&(selectedCost()>available()||totalPlanned()>MAX_POLICIES))S.selected.pop();
}
function milestone(){
  return S.year===1?'🌱 1年目：データを読んで、2〜3つの施策を組み合わせよう。':
    S.year===3?'🏁 3年目：折り返し。続ける事業とやめる事業を見直そう。':
    S.year===5?'🏆 最終年：何を残し、何を手放すか。':'あと'+(6-S.year)+'年。継続も含めて、動かせる施策は最大3つです。';
}
function plan(){
...
function renderPolicies(){
  const h=$('policyGrid');h.innerHTML='';
  P.forEach(p=>{
    if(p.id==='rest')return;
    const active=S.active.includes(p.id),sel=S.selected.includes(p.id),fit=localFit(p),full=totalPlanned()>=MAX_POLICIES&&!sel&&!active,d=document.createElement('div');
    ...
    const status=active?'<span class="activeBadge">継続判断は上で</span>':full?'<span class="activeBadge">今年の3枠が埋まっています</span>':'';
...
function toggle(id){
  ...
  if(i>=0)S.selected.splice(i,1);
    ...
    if(totalPlanned()>=MAX_POLICIES)return;
...
function renderSelected(){
  const a=S.selected.map(policy),spent=selectedCost(),remain=Math.max(0,available()-spent),continued=continuedIds().length,total=totalPlanned(),left=slotsLeft();
  $('budget').textContent=remain;
  $('upkeepBox').innerHTML='<b>今年の施策枠 '+total+'/'+MAX_POLICIES+'（継続 '+continued+'／新規 '+a.length+'）</b><br><span class="small">継続費 '+upkeep()+'pt。続ける事業を「やめる」にすると、その年から維持費と継続効果がなくなり、施策枠も1つ空きます。</span>';
  $('selectedSummary').innerHTML=a.length?'<b>新しく選択：</b>'+a.map(p=>p.icon+p.name).join(' ＋ ')+'<br><span class="small">新規費 '+spent+'pt／残り '+remain+'pt／あと'+left+'枠</span>':'<b>新規施策はまだありません。</b><br><span class="small">継続を含めて今年は'+total+'/'+MAX_POLICIES+'枠を使用中です。</span>';
  const dock=$('runDock');
  if(total>=2&&total<=MAX_POLICIES&&!S.resolved){
    dock.classList.add('show');
    $('dockTitle').textContent='今年の施策 '+total+'/'+MAX_POLICIES;
    $('dockSummary').textContent=(continued?'継続'+continued+' ＋ ':'')+(a.length?'新規'+a.length:'新規0')+(left?'　あと'+left+'つ追加できます':'　3枠すべて使用');
    $('dockRun').textContent='この'+total+'施策で1年進める →';
  }else dock.classList.remove('show');
}
function renderAll(){
  $('year').textContent=S.year;$('milestone').textContent=milestone();renderContinuations();renderSelected();renderEvent();renderMetrics();renderPolicies();
}
function runYear(){
  const total=totalPlanned();
  if(S.resolved||total<2||total>MAX_POLICIES)return;
  ...
  const allNames=continuedNow.map(id=>'継続:'+policy(id).name).concat(chosen.map(p=>'新規:'+p.name));
  S.history.push({year:S.year,names:allNames,v:{...S.v}});
  const g=GOALS[S.goal];
  $('resultTitle').textContent=S.year+'年目の結果';$('surprise').textContent='予想外の出来事：'+surprise.text;
  $('changes').innerHTML=Object.keys(M).map(k=>{
    const d=S.v[k]-before[k],target=(k==='fatigue'?'≤':'≥')+g.t[k];
    return '<div class="change">'+M[k].i+' '+M[k].n+' <b class="'+(d>=0?'up':'down')+'">'+(d>0?'+':'')+d+'</b> → '+S.v[k]+' <span class="targetNote">（目標値: '+target+'）</span></div>';
  }).join('');
...
function renderHistory(){
  $('history').innerHTML=S.history.map(h=>'<tr><td>'+h.year+'</td><td>'+h.names.join(' / ')+'</td>'+['connection','youth','child','senior','stay','industry','tourism','equity','fatigue'].map(k=>'<td>'+h.v[k]+'</td>').join('')+'</tr>').join('');
}
function next(){
  if(S.year>=5)return finish();S.year++;S.selected=[];S.event=null;S.currentBudgetBonus=S.nextBudgetBonus;S.nextBudgetBonus=0;S.resolved=false;S.keep={};S.active.forEach(id=>S.keep[id]=true);$('result').classList.add('hidden');renderAll();window.scrollTo({top:0,behavior:'smooth'});
}
function finish(){
  const g=GOALS[S.goal],sc=[];Object.entries(g.t).forEach(([k,t])=>sc.push(k==='fatigue'?Math.min(1,t/Math.max(1,S.v[k])):Math.min(1,S.v[k]/t)));
  let ach=Math.round(sc.reduce((a,b)=>a+b,0)/sc.length*100);if(S.approach==='hybrid')ach=Math.min(100,ach+3);
  ...
  $('endingLabel').textContent=ach>=90?'🏆 一貫したまちづくり':ach>=78?'🌟 方向性が見えてきた':ach>=65?'🧭 成果と課題がはっきりした':'🧪 データの読み方から再挑戦';
  ...
  $('challengeReport').innerHTML=c.slice(0,5).map(x=>'<li>'+x+'</li>').join('');$('endingQuestion').innerHTML='<b>最後の問い：</b>3つしか動かせない中で、何を続け、何をやめ、何を新しく始めたか。その優先順位は5年後の町にどう表れましたか。';window.scrollTo({top:0,behavior:'smooth'});
}
function start(){
